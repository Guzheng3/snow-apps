...
def _byte_array_expression(value: str, index: int) -> str:
    # MSVC rejects individual string literals larger than roughly 16 KiB.
    # Keep chunks comfortably below that limit while preserving UTF-8 code points.
    chunks: list[str] = []
    current: list[str] = []
    current_bytes = 0
...
def _colors_expression(defaults: dict[str, str]) -> str:
    if not defaults:
        return "{}"
    expression = "IconColors()"
    for slot_name in ("primary", "secondary", "tertiary"):
        if slot_name in defaults:
            expression += f'.with{slot_name.title()}(QColor({_cpp_string(defaults[slot_name])}))'
    return expression
...
def _render_source(data: dict[str, Any], entries: list[Entry], header_name: str) -> str:
    ...
    lines = [
        "// Generated by tools/generate_icon_pack.py. DO NOT EDIT.",
        f'#include "{header_include}"', "", "#include <QColor>", "", f"namespace {namespace} {{", "", "namespace {", "",
        "adqt::icons::ExternalIconPackDefinition definition() {",
        "  using namespace adqt::icons;",
        "  ExternalIconPackDefinition result;",
        f"  result.pack = QStringLiteral({_cpp_string(data['pack'])});",
        f"  result.source = QStringLiteral({_cpp_string(data.get('source', 'project-owned manifest'))});",
        f"  result.contentHash = QByteArrayLiteral({_cpp_string(pack_hash)});",
        f"  result.entries.reserve({len(entries)});",
    ]
    for index, entry in enumerate(entries):
        lines.extend([
            "  result.entries.append(ExternalIconPackEntry{",
            f"      QStringLiteral({_cpp_string(entry.variant)}), QStringLiteral({_cpp_string(entry.name)}),",
            f"      {MODELS[entry.model]}, {FITS[entry.fit]}, {_colors_expression(entry.defaults)},",
            f"      {_byte_array_expression(entry.normalized_svg, index)},",
            f"      QByteArrayLiteral({_cpp_string(entry.source_hash)}), {'true' if entry.allow_data_images else 'false'}}});",
        ])
    lines.extend([
    ...
    for entry in entries:
        ...
        lines.extend([
            ...
            f"  return pack().icon(QStringLiteral({_cpp_string(entry.variant)}), QStringLiteral({_cpp_string(entry.name)}), colors);",
            "}", "",
        ])
    if current_variant is not None:
        lines.extend([f"}}  // namespace {current_variant}", ""])
