...
const WGC_DIRTY_RECT_DENSE_MERGE_MIN_RECTS: usize = 64;
const WGC_DIRTY_RECT_DENSE_MERGE_MAX_VERTICAL_SPAN: u32 = 96;
...
const WGC_DIRTY_RECT_DENSE_MERGE_THRESHOLDS: DirtyRectDenseMergeThresholds =
    DirtyRectDenseMergeThresholds {
        min_rects: WGC_DIRTY_RECT_DENSE_MERGE_MIN_RECTS,
        max_vertical_span: WGC_DIRTY_RECT_DENSE_MERGE_MAX_VERTICAL_SPAN,
    };

#[inline(always)]
fn texture_desc_matches(lhs: D3D11_TEXTURE2D_DESC, rhs: D3D11_TEXTURE2D_DESC) -> bool {
    lhs.Width == rhs.Width
...
fn should_short_circuit_duplicate(
    ...
    destination_has_history && source_is_duplicate && pending_slot_available
}

#[inline(always)]
fn frame_pool_buffers_for_mode(mode: CaptureMode) -> i32 {
    match mode {
        CaptureMode::Snapshot => WGC_FRAME_POOL_BUFFERS_SCREENSHOT,
        CaptureMode::Continuous => WGC_FRAME_POOL_BUFFERS_RECORDING,
    }
}
...
impl WindowsGraphicsCaptureCapturer {
    fn new(
        ...
        let stale_timeout_config = stale_timeout_config(false);
        ...
        let capture_mode = CaptureMode::Snapshot;
        let frame_pool_buffer_count = frame_pool_buffers_for_mode(capture_mode);
    ...
    fn recreate_pool_if_needed(&mut self, frame: &Direct3D11CaptureFrame) -> CaptureResult<()> {
        ...
        let desired_buffer_count = frame_pool_buffers_for_mode(self.capture_mode);
        if content_size.Width != self.pool_size.Width
            || content_size.Height != self.pool_size.Height
            || desired_buffer_count != self.frame_pool_buffer_count
        {
            self.frame_pool
                .Recreate(
                    &self.winrt_device,
            ...
            self.frame_pool_buffer_count = desired_buffer_count;
    ...
    fn effective_source(
        ...
        if src_desc.Format != DXGI_FORMAT_R16G16B16A16_FLOAT {
            return Ok((frame_texture.clone(), src_desc, self.hdr_to_sdr));
        }
        ...
        Ok((frame_texture.clone(), src_desc, self.hdr_to_sdr))
    }
    ...
    fn capture_region_into(
        ...
        let pipeline_reuse_enabled = destination_has_history;
        let allow_stale_return = pipeline_reuse_enabled;
        ...
        ) && let Some(sample) = self.try_short_circuit_region_duplicate(capture_time, time_ticks)
        {
        ...
        let capture_result = (|| -> CaptureResult<CaptureSampleMetadata> {
            ...
            let (effective_source, effective_desc, effective_hdr) =
                self.effective_source(&frame_texture, src_desc)?;
            ...
            let recording_mode = pipeline_reuse_enabled;
            ...
            let reuse_pending_slot_for_dirty_gpu = self.capture_mode != CaptureMode::Continuous
                ...
                && self.region.pending_slot.is_some()
                && self.region.pending_slot.is_some_and(|slot_idx| {
                    let slot = &self.region.slots[slot_idx];
                    slot.populated
                        && slot
                            .source_desc
                            .is_some_and(|cached| texture_desc_matches(cached, region_desc))
            ...
            let read_slot = if skip_submit_copy {
                ...
                slot.populated = true;
                slot_idx
            } else if reuse_pending_slot_for_dirty_gpu {
                let slot_idx = self.region.pending_slot.unwrap_or(read_slot);
                self.ensure_region_slot(slot_idx, &region_desc)?;
                {
                    let slot = &mut self.region.slots[slot_idx];
                    slot.capture_time = Some(capture_time);
                    slot.present_time_ticks = time_ticks;
                    ...
                    slot.dirty_cpu_copy_preferred = strategy.cpu;
                    slot.dirty_gpu_copy_preferred = strategy.gpu;
                ...
                d3d11::with_texture_resource(
                    ...
                    "failed to cast WGC region source texture to ID3D11Resource",
                    |source_resource| {
                        self.copy_region_source_to_slot(slot_idx, source_resource, blit, true)
                    },
                )?;
                self.maybe_flush_region_after_submit(slot_idx, slot_idx);
                slot_idx
            } else {
            ...
            if recording_mode {
                if !skip_submit_copy {
                    let next_slot = if reuse_pending_slot_for_dirty_gpu {
                        read_slot
                    } else {
                        write_slot
                    };
                    self.region.pending_slot = Some(next_slot);
    ...
    fn capture_with_history_hint(
        ...
        let pipeline_reuse_enabled = destination_has_history;
        let allow_stale_return = pipeline_reuse_enabled && out.width() > 0 && out.height() > 0;
        let capture_time = Instant::now();
        ...
        let capture_result = (|| -> CaptureResult<()> {
            ...
            let read_slot = if pipeline_reuse_enabled {
            ...
            let reuse_pending_slot_for_dirty_gpu = self.capture_mode != CaptureMode::Continuous
                ...
                && source_dirty_available
                && self.pending_slot.is_some()
                && self.pending_slot.is_some_and(|slot_idx| {
                    let slot = &self.staging_slots[slot_idx];
                    slot.populated
                        && slot
                            .source_desc
                            .is_some_and(|cached| texture_desc_matches(cached, effective_desc))
                        && slot.hdr_to_sdr == effective_hdr
                });
            ...
            let read_slot = if skip_submit_copy {
                ...
                let slot_idx = self.pending_slot.unwrap_or(read_slot);
                self.ensure_staging_slot(slot_idx, &effective_desc)?;
                {
                    let slot = &mut self.staging_slots[slot_idx];
                    slot.dirty_mode_available = true;
                ...
                d3d11::with_texture_resource(
                    &effective_source,
                    "failed to cast WGC frame texture to ID3D11Resource",
                    |source_resource| self.copy_source_to_slot(slot_idx, source_resource, true),
                )?;
                self.maybe_flush_after_submit(slot_idx, slot_idx);
                slot_idx
            } else {
    ...
    fn set_capture_mode(&mut self, mode: CaptureMode) {
        ...
        let desired_buffer_count = frame_pool_buffers_for_mode(mode);
        if desired_buffer_count != self.frame_pool_buffer_count
            && self
                .frame_pool
                .Recreate(
                    &self.winrt_device,
        ...
        self.reset_staging_pipeline();
        match mode {
            CaptureMode::Continuous => self.warm_recording_scratch(),
            CaptureMode::Snapshot => self.trim_scratch_for_screenshot(),
        }
    }
...
mod tests {
    ...
    #[test]
    fn duplicate_short_circuit_supports_hot_screenshot_sessions() {
    ...
    #[test]
    fn region_duplicate_short_circuit_supports_hot_screenshot_sessions() {
    ...
    fn clamp_dirty_region_rect_baseline(
        raw: RectInt32,
        width: u32,
        height: u32,
    ) -> Option<DirtyRect> {
    ...
    fn normalize_raw_dirty_rects_baseline(
        raw_rects: &[RectInt32],
        width: u32,
        height: u32,
        out: &mut Vec<DirtyRect>,
        ...
        for raw in raw_rects {
            if let Some(clamped) = clamp_dirty_region_rect_baseline(*raw, width, height) {
                out.push(clamped);
            }
        }
    ...
    fn extract_region_raw_dirty_rects_baseline(
        raw_rects: &[RectInt32],
        source_width: u32,
        source_height: u32,
        blit: CaptureBlitRegion,
        ...
        out.reserve(raw_rects.len());
        for raw in raw_rects {
            let Some(clamped) = clamp_dirty_region_rect_baseline(*raw, source_width, source_height)
            else {
                continue;
            };
    ...
    fn extract_full_raw_dirty_rects_baseline(
        raw_rects: &[RectInt32],
        width: u32,
        height: u32,
        out: &mut Vec<DirtyRect>,
    ...
    #[test]
    fn dirty_region_rect_clamp_matches_baseline_logic() {
        ...
        for _ in 0..1024 {
            ...
            let baseline = clamp_dirty_region_rect_baseline(raw, 1920, 1080);
    ...
    #[test]
    fn region_dirty_extraction_matches_baseline_when_dense_fallback_is_disabled() {
        ...
        let baseline_state =
            extract_region_raw_dirty_rects_baseline(&raw_rects, 1920, 1080, blit, &mut baseline);
    ...
    #[test]
    fn region_dirty_extraction_matches_baseline_for_out_of_bounds_rects_when_dense_fallback_disabled()
     {
        let blit = CaptureBlitRegion {
            src_x: 0,
            src_y: 0,
        ...
        for case_idx in 0..256 {
            ...
            let mut baseline = Vec::new();
            let mut optimized = Vec::new();
            let baseline_state = extract_region_raw_dirty_rects_baseline(
                &raw_rects,
                1920,
                1080,
                blit,
            ...
            assert_eq!(
                baseline_state, optimized_state,
    ...
    fn region_dirty_extraction_dense_fallback_preserves_full_copy_decision() {
        ...
        assert!(baseline_state.available);
        assert!(!baseline_state.unchanged);
        let baseline_strategy = evaluate_dirty_copy_strategy(&baseline, blit.width, blit.height);
        assert_eq!(baseline_strategy, DirtyCopyStrategy::default());
    ...
    #[test]
    fn full_dirty_extraction_matches_baseline_when_dense_fallback_is_disabled() {
        ...
        let mut optimized = Vec::new();
        let baseline_state =
            extract_full_raw_dirty_rects_baseline(&raw_rects, 1920, 1080, &mut baseline);
        let optimized_state =
    ...
    #[test]
    fn full_dirty_extraction_matches_baseline_for_out_of_bounds_rects_when_dense_fallback_disabled()
    {
    ...
    fn full_dirty_extraction_dense_fallback_preserves_full_copy_decision() {
        ...
        assert!(baseline_state.available);
        assert!(!baseline_state.unchanged);
        let baseline_strategy = evaluate_dirty_copy_strategy(&baseline, 1920, 1080);
        assert_eq!(baseline_strategy, DirtyCopyStrategy::default());
    ...
    #[test]
    #[ignore = "performance benchmark guard; run explicitly with --ignored --nocapture"]
    fn bench_dirty_region_rect_clamp_and_normalize_vs_baseline() {
        use std::hint::black_box;
        ...
        let mut baseline = Vec::<DirtyRect>::new();
        let mut optimized = Vec::<DirtyRect>::new();
        for rects in &workloads {
            normalize_raw_dirty_rects_baseline(rects, 1920, 1080, &mut baseline);
        ...
        println!(
            "wgc dirty-rect clamp benchmark: baseline={baseline_ms:.3} ms optimized={optimized_ms:.3} ms speedup={speedup:.2}x"
        );

        assert!(
            optimized_ms <= baseline_ms * 1.05,
            "dirty-rect clamp/normalize regressed: baseline={baseline_ms:.3}ms optimized={optimized_ms:.3}ms ({speedup:.2}x)"
        );
    }

    #[test]
    #[ignore = "performance benchmark guard; run explicitly with --ignored --nocapture"]
    fn bench_region_dirty_dense_fallback_vs_baseline() {
        use std::hint::black_box;
        ...
        let mut baseline_rects = Vec::<DirtyRect>::new();
        ...
        for raw_rects in &workloads {
            ...
            let baseline_strategy =
                evaluate_dirty_copy_strategy(&baseline_rects, blit.width, blit.height);
            assert!(baseline_state.available);
            assert!(!baseline_state.unchanged);
            assert_eq!(
                baseline_strategy,
        ...
        for iter in 0..warmup_iters {
            ...
            let baseline_strategy =
                evaluate_dirty_copy_strategy(&baseline_rects, blit.width, blit.height);
            sink ^= baseline_rects.len();
            sink ^= baseline_state.available as usize;
            sink ^= (baseline_strategy.cpu as usize) << 1;
        ...
        println!(
            "wgc region dense-fallback benchmark: baseline={baseline_ms:.3} ms optimized={optimized_ms:.3} ms speedup={speedup:.2}x"
        );

        assert!(
            optimized_ms <= baseline_ms * 0.90,
            "dense-fallback path did not improve enough: baseline={baseline_ms:.3}ms optimized={optimized_ms:.3}ms ({speedup:.2}x)"
        );
    }

    #[test]
    #[ignore = "performance benchmark guard; run explicitly with --ignored --nocapture"]
    fn bench_full_dirty_dense_fallback_vs_baseline() {
        use std::hint::black_box;
        ...
        for raw_rects in &workloads {
            let baseline_state =
                extract_full_raw_dirty_rects_baseline(raw_rects, 1920, 1080, &mut baseline_rects);
            let baseline_strategy = evaluate_dirty_copy_strategy(&baseline_rects, 1920, 1080);
            assert!(baseline_state.available);
            assert!(!baseline_state.unchanged);
            assert_eq!(
                baseline_strategy,
        ...
        println!(
            "wgc full-frame dense-fallback benchmark: baseline={baseline_ms:.3} ms optimized={optimized_ms:.3} ms speedup={speedup:.2}x"
        );

        assert!(
            optimized_ms <= baseline_ms * 0.90,
            "full-frame dense-fallback path did not improve enough: baseline={baseline_ms:.3}ms optimized={optimized_ms:.3}ms ({speedup:.2}x)"
        );
    }
    ...
    fn bench_dirty_copy_strategy_single_pass_vs_baseline() {
        ...
        println!(
            "wgc dirty strategy benchmark: baseline={baseline_ms:.3} ms optimized={optimized_ms:.3} ms speedup={speedup:.2}x"
        );
    ...
    #[test]
    fn aggressive_stale_timeout_profile_is_tighter_than_baseline_profile() {
        let baseline = stale_timeout_config(false);
        let aggressive = stale_timeout_config(true);
        assert!(aggressive.initial < baseline.initial);
        assert!(aggressive.max < baseline.max);
        assert!(aggressive.increase_step <= baseline.increase_step);
        assert!(aggressive.decrease_step <= baseline.decrease_step);
        assert_eq!(aggressive.min, baseline.min);
    }
